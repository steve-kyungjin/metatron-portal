select 
	...
	,exec_log.exec_type
	,exec_log.exec_detail	
	,exec_log.exec_search_keyword
	,exec_log.exec_search_count
	,user_info.user_id
	,user_info.user_name
...
from ( 
	select 
		...
		,replace(var,concat(',',substring_index(var,',',-1)),'') as exec_search_keyword
		,substring_index(var, ',', -1) as exec_search_count
		,user_id
		,time
	from metatron_portal.mp_log_action 
	where date_format(time, '%Y-%m-%d') >= '2018-10-05'
		and type = 'SEARCH'
		and action = 'VIEW' 
) as exec_log
inner join 
...
select 
	...
	,exec_log.exec_date
	,exec_log.exec_weekday_cd
	,case when exec_weekday_cd = 0 then 'SUN'
	      when exec_weekday_cd = 1 then 'MON'
	      when exec_weekday_cd = 2 then 'TUE'
	      when exec_weekday_cd = 3 then 'WED'
	      when exec_weekday_cd = 4 then 'THU'
	      when exec_weekday_cd = 5 then 'FRI'		  
	      when exec_weekday_cd = 6 then 'SAT'	
	 end as exec_weekday	
	,exec_log.exec_hours
	,exec_log.exec_hourmin
	,exec_log.exec_hostname
...
from ( 
	select 
		...
		,host as exec_hostname
		,system as exec_system
		,var as exec_search_keyword
		,null as exec_search_count
		,user_id
		,time
	from metatron_portal.mp_log_action 

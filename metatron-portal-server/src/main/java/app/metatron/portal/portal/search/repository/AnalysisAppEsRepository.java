...
@Slf4j
@Repository
public class AnalysisAppEsRepository {

    @Autowired
    private TransportClient client;

    @Autowired
    public AnalysisAppEsRepository(TransportClient client) {
        this.client = client;
    }
    ...
    public void setBulk(List<AnalysisAppIndexVO> list) {
        ...
        list.forEach( vo -> {
            try {
                ...
                brb.add(client.prepareIndex(Const.ElasticSearch.INDEX,Const.ElasticSearch.TYPE_ANALYSIS_APP,vo.getId()).setSource(mapper.writeValueAsString(vo)));
            }catch (Exception e){

...
@Slf4j
@Repository
public class CommunicationEsRepository {

    @Autowired
    private TransportClient client;

    @Autowired
    public CommunicationEsRepository(TransportClient client) {
        this.client = client;
    }
    ...
    public Map<String, Object> findString(String str, Pageable pageable) {
        ...
        QueryBuilder queryBuilder = QueryBuilders.boolQuery()
                .should(QueryBuilders.matchQuery("postTitle", str))
                .should(QueryBuilders.matchQuery("postContent", str))
                .should(QueryBuilders.matchQuery("displayNm", str));
        ...
        try{
            response = client.prepareSearch(Const.ElasticSearch.INDEX)
                    .setTypes(Const.ElasticSearch.TYPE_COMMUNICATION)
    ...
    public void setBulk(List<CommunicationIndexVO> list) {
        if( null == client ){
            return;
        }
        ...
        list.forEach( vo -> {
            try {
                ...
                brb.add(client.prepareIndex(Const.ElasticSearch.INDEX,Const.ElasticSearch.TYPE_COMMUNICATION,vo.getId()).setSource(mapper.writeValueAsString(vo)));
            }catch (Exception e){

...
@Slf4j
@Repository
public class ProjectEsRepository {

    @Autowired
    private TransportClient client;

    @Autowired
    public ProjectEsRepository(TransportClient client) {
        this.client = client;
    }
    ...
    public Map<String, Object> findString(String str, Pageable pageable) {
        ...
        QueryBuilder queryBuilder = QueryBuilders.boolQuery()
                ...
                .should(QueryBuilders.matchQuery("summary", str))
                .should(QueryBuilders.matchQuery("progress", str));

        Map<String ,Object> result = new HashMap<>();
        SearchResponse response = null;
        try{
            response = client.prepareSearch(Const.ElasticSearch.INDEX)
                    .setTypes(Const.ElasticSearch.TYPE_PROJECT)
    ...
    public void setBulk(List<ProjectIndexVO> list) {
        if( null == client ){
            return;
        }
        ...
        list.forEach( vo -> {
            try {
                ...
                brb.add(client.prepareIndex(Const.ElasticSearch.INDEX,Const.ElasticSearch.TYPE_PROJECT,vo.getId()).setSource(mapper.writeValueAsString(vo)));
            }catch (Exception e){

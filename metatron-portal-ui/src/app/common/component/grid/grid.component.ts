...
export class GridComponent implements AfterViewInit, OnDestroy {
	 ...
	 * CSV file download
	 */
	public customAppCsvDownload(fileName: string = ''): void {
		...
		this.fields
			.forEach((headerName, index) => {
				header.push('"' + headerName + '(' + this.fieldKoNames[ index ] + ')' + '"');
			});
		...
		this.getRows()
			.forEach((column) => {
				const obj: any[] = [];
				this.fields.forEach(headerName => {
	...
	public create(headers: header[], rows: any[], option: Option = null): boolean {
		...
		try {
			...
			if (true === option.frozenTotal) {
				rows.forEach((row, index) => {
					if (row._idProperty_ !== this.GRID_TOTAL_ID) {
						row._idProperty_ = index + 1
					}
					return row;
				});
			} else {
				rows.forEach((row, index) => {
					return row._idProperty_ = index + 1
				});
			}
			...
			// setCellCssStyles이 true일 경우 null처리를 해준다.
			if (this.option.enableHeaderClick && this.option.nullCellStyleActivate) {
	...
	public selectColumn(column: number | string, isSelectOrToggle: boolean | string, scope: any = null, isShiftKeyPressed?: boolean, isCtrlKeyPressed?: boolean, type?: string): void {
		...
		if ('string' === typeof column) {
			columnId = column;
			columnIdx = fnScope.grid.getColumnIndex(columnId);
		} else if ('number' === typeof column) {
			columnIdx = column;
			columnId = fnScope.grid.getColumns()[ columnIdx ].id;
		} else {
		...
		// 이벤트 발생
		fnScope.grid.getColumns()[ columnIdx ][ 'select' ] = isSelect;
	...
	private createSearchFields(headers: header[]): void {
		...
		headers.forEach((header) => {
			...
			const fieldKoName: string = header[ 'name' ];
			if (!('undefined' === typeof fieldKoName)) {
				this.fieldKoNames.push(fieldKoName);
			}
		});
	}
	...
	private createGrid(headers: header[], rows: any[]): void {
		...
		if (this.option.enableHeaderMenu) {
			...
			// Header menu plugin
			let headerMenuPlugin = new Slick.Plugins.HeaderMenu({});
			headerMenuPlugin.onCommand.subscribe(
				(function (scope) {
					return function (e, args) {
						scope.selectedHeaderMenuEvent.emit({ event: null, args: args });
					};
				})(this)
			);
					...
					return function (event, args) {
						try {
							...
							if (true === scope.option.frozenTotal) {
								...
								const rowsHeight: number = scope.getRowsHeight();

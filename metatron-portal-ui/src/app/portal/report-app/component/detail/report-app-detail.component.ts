...
@Component({
	selector: 'report-app-detail',
	templateUrl: './report-app-detail.component.html'
})
export class ReportAppDetailComponent extends AbstractComponent implements OnInit, OnDestroy {
	...
	// 앱 상세
	public reportApp: ReportApp.Entity = new ReportApp.Entity();
	// 앱 카테고리 목록
	...
	// 리뷰 목록
	public reviews: Array<ReportApp.Review>;
	// 리뷰 작성폼 show/hide
	...
	public init() {
		...
		this.reportAppService
			...
			.then(result => {
				...
				this.reportApp = result.data.reportApp;
				...
				// 이미지 목록
				if (this.reportApp.mediaGroup) {
					const medias = this.reportApp.mediaGroup.medias;
					this.imageList = [];
				...
				// 카테고리 목록
				this.categories = this.reportApp.categories;
				...
				// 태깅
				this.tagging(this.TaggingType.DETAIL, this.TaggingAction.VIEW, this.reportApp.id, this.reportApp.appNm);
	...
	public requestRoleClick() {
		this.roleService.requestAppRole('REPORT_APP', this.appId).then(result => {
			if (result.code === CommonConstant.CODE.RESULT_CODE.SUCCESS) {
				Alert.success(this.translateService.instant('COMMON.MESSAGE.CREATE', '등록되었습니다.'));
			} else {
				Alert.error(this.translateService.instant('ANALYSIS.APP.DETAIL.ROLE.POPUP.ALREADY', '오류'));
			}
	...
	public appDeleteClick() {
		this.dialogService.confirm(
			...
			() => {
				this.reportAppService.deleteMyApp(this.appId).then(result => {
					if (result.code === CommonConstant.CODE.RESULT_CODE.SUCCESS) {
	...
	public appExecClick() {
		...
		if (this.reportApp.externalYn && _.isUndefined(this.reportApp.urlHeader) === false) {
			window.open(this.reportApp.urlHeader.navigation, this.reportApp.id);
			return;
		}

		this.router.navigate([ `view/report-app/my-app/${this.reportApp.id}` ]);
	}
	 ...
	 */
	public appAddClick() {
		this.reportAppService.addMyApp(this.appId).then(result => {
			if (result.code === CommonConstant.CODE.RESULT_CODE.SUCCESS) {
				Alert.success(this.translateService.instant('COMMON.MESSAGE.CREATE', '등록되었습니다.'));
				this.appAuth = AppAuth.ADDED;
				this.reportApp.usage++;
	...
	public reviewCreateClick() {
		...
		let review = new ReportApp.Review();
		review.contents = this.reviewText;
		this.reportAppService.createReview(this.appId, review).then(result => {
			if (result.code === CommonConstant.CODE.RESULT_CODE.SUCCESS) {
				this.reviewText = '';
				this.showReviewForm = false;
	 ...
	 * 리뷰 취소 클릭
	 */
	public reviewCancelClick(item: ReportApp.Review) {
		item.openMenu = false;
		item.isView = true;
	 ...
	 * 리뷰 저장 클릭
	 */
	public reviewSaveClick(item: ReportApp.Review) {
		item.openMenu = false;
		this.reportAppService.updateReview(item).then(result => {
			if (result.code === CommonConstant.CODE.RESULT_CODE.SUCCESS) {
	 ...
	 * 리뷰/답변 삭제 클릭
	 */
	public reviewDeleteClick(item: ReportApp.Review) {
		item.openMenu = false;
		this.dialogService.confirm(
			this.translateService.instant('COMMON.DELETE', '삭제'),
			this.translateService.instant('COMMON.MESSAGE.CONFIRM.DELETE', '삭제'),
			...
			() => {
				...
				this.reportAppService.deleteReview(item).then(result => {
					if (result.code === CommonConstant.CODE.RESULT_CODE.SUCCESS) {
						Alert.success(this.translateService.instant('COMMON.MESSAGE.DELETE', '삭제되었습니다.'));
	 ...
	 * 답변 취소 클릭
	 */
	public replyCancelClick(item: ReportApp.Review) {
		item.openMenu = false;
		if (!item.id) {
			this.dialogService.confirm(
				this.translateService.instant('COMMON.CANCEL', '취소'),
	 ...
	 * 답변 저장 클릭
	 */
	public replySaveClick(item: ReportApp.Review) {
		item.openMenu = false;
		if (!item.id) {
			this.reportAppService.createReview(this.appId, item).then(result => {
				if (result.code === CommonConstant.CODE.RESULT_CODE.SUCCESS) {
					Alert.success(this.translateService.instant('COMMON.MESSAGE.CREATE', '등록되었습니다.'));
	...
	public selectCategory(item: Code.Entity) {
		if (item.id) {
			this.router.navigate([ `/view/report-app` ], { queryParams: { category: item.id } });
		} else {
			this.router.navigate([ `/view/report-app` ]);
		}
	}
	...
	public appClick(item: ReportApp.Entity) {
		if (item.id) {
			this.logger.debug(`view/report-app/${item.id}`);
				...
				if (result.code === CommonConstant.CODE.RESULT_CODE.SUCCESS) {
					...
					for (let i = 0; i < this.reviews.length; i++) {
						...
						if (!this.reviews[ i ].children.length) {
							let reply = new ReportApp.Review();
							reply.user = this.appSession.user;
	...
	private checkEnableAnswer() {
		const userId = this.cookieService.get(CookieConstant.KEY.USER_ID);
		if ((this.reportApp.createdBy && userId === this.reportApp.createdBy.userId) || this.isAdmin) {

...
@Component({
	selector: 'report-app-my-app',
	templateUrl: './my-app.component.html'
})
export class ReportAppMyAppComponent extends AbstractComponent implements OnInit, OnDestroy {
		...
		this.reportAppService
			...
			.then(result => {
				...
				if (result.code === CommonConstant.CODE.RESULT_CODE.SUCCESS) {
					...
					const data: ReportApp.MyAppMain = result.data;
					// 마이 앱 목록
					this.appList = [];
					//
	 ...
	 * 삭제 클릭
	 */
	public appDeleteClick(app: ReportApp.Entity) {
		this.dialogService.confirm(
			...
			() => {
				this.reportAppService.deleteMyApp(app.id).then(result => {
					if (result.code === CommonConstant.CODE.RESULT_CODE.SUCCESS) {
	...
	public search(isInit: boolean = false) {
		...
		this.reportAppService.getMyAppList(this.searchText, this.pageable).then(result => {
			// 초기화
			if (isInit) {
				this.appList = [];
			}

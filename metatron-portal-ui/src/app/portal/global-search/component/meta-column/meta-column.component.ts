...
@Component({
	...
	templateUrl: './meta-column.component.html'
})
export class MetaColumnComponent extends BaseComponent implements OnInit, OnDestroy {
	 ...
	 * 검색 타입 인덱스 가져오기
	 */
	public currentGlobalSearchTypeEnumIndex = getTypeIndex(GlobalSearch.Type.META_COLUMN);
	...
	public ngOnInit(): void {
		...
		this.subscriptions.push(
			this.activatedRoute
				...
				.subscribe(params => {
					this.pageReset();
					this.globalSearchService.searchType = GlobalSearch.Type.META_COLUMN;
					const q = params[ 'q' ];
	...
	private search(keyWord: string): void {
		...
		if (_.isUndefined(keyWord) === false && _.isEmpty(keyWord) === false) {
			...
			Promise.resolve()
				...
				.then(() => {
					this.globalSearchService
						.executeGlobalSearchByType(keyWord, GlobalSearch.Type.META_COLUMN, this.page)
						.then((result: any) => {
							...
							if (result.code === CommonConstant.CODE.RESULT_CODE.SUCCESS) {
								...
								this.metaColumnEntity = result.data.result;
							} else {
								this.pagingInit();
								this.metaColumnEntity.contentsList = [];
							}

...
export class ReportAppComponent extends BaseComponent implements OnInit, OnDestroy {
	 ...
	 * 검색 타입 인덱스 가져오기
	 */
	public currentGlobalSearchTypeEnumIndex = getTypeIndex(GlobalSearch.Type.REPORT_APP);
	...
	public ngOnInit(): void {
		...
		this.subscriptions.push(
			this.activatedRoute
				...
				.subscribe(params => {
					this.pageReset();
					this.globalSearchService.searchType = GlobalSearch.Type.REPORT_APP;
					const q = params[ 'q' ];
	...
	private search(keyWord: string): void {
		...
		if (_.isUndefined(keyWord) === false && _.isEmpty(keyWord) === false) {
			...
			Promise.resolve()
				...
				.then(() => {
					this.globalSearchService
						.executeGlobalSearchByType(keyWord, GlobalSearch.Type.REPORT_APP, this.page)
						.then((result: GlobalSearch.Result.ReportAppList) => {
							...
							if (result.code === CommonConstant.CODE.RESULT_CODE.SUCCESS) {
								...
								this.reportAppEntity = result.data.result;
							} else {
								this.pagingInit();
								this.reportAppEntity.contentsList = [];
							}

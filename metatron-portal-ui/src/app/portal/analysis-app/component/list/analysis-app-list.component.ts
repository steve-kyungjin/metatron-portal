...
@Component({
	selector: 'analysis-app-list',
	templateUrl: './analysis-app-list.component.html'
})
export class AnalysisAppListComponent extends AbstractComponent implements OnInit, OnDestroy {
	...
	// 실행 앱 목록
	public execAppList: Array<AnalysisApp.Entity>;
	// 정렬 목록
		...
		this.analysisAppService
			...
			.then(result => {
				if (result.code === CommonConstant.CODE.RESULT_CODE.SUCCESS) {
					...
					// 앱 목록
					this.appList = [];
					this.setList(data.analysisAppList);
					// 카테고리 목록
					...
					for (let i = 0; i < this.latestAppList.length; i++) {
						let img: SwiperSlide = new SwiperSlide(
							this.analysisAppService.getFirstImage(this.latestAppList[ i ], true),
	...
	public search(isInit: boolean = false) {
		...
		this.analysisAppService
			...
			.then(result => {
				...
				// 앱 목록
				this.setList(result.data.analysisAppList);
